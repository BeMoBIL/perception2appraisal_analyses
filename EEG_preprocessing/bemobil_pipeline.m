...
input_filepath = fullfile(bemobil_config.study_folder, bemobil_config.raw_EEGLAB_data_folder, [bemobil_config.filename_prefix num2str(subject)]);
output_filepath = string(fullfile(bemobil_config.study_folder, bemobil_config.single_subject_analysis_folder, [bemobil_config.filename_prefix num2str(subject)]));
...
% this example removes everything before the first and after the last event, plus it searches for the pause keyword
% to remove breaks with a buffer of 1 second

allevents = {EEG.event.type}';

% I ADDED THIS PART
% Find the first non-'R-peak' event
first_non_r_peak_idx = find(~strcmp(allevents, 'R-peak'), 1, 'first');

% Find the last non-'R-peak' event
last_non_r_peak_idx = find(~strcmp(allevents, 'R-peak'), 1, 'last');

% Initialize removeindices
removeindices = [];

% If both indices are found, set the remove indices accordingly
if ~isempty(first_non_r_peak_idx) && ~isempty(last_non_r_peak_idx)
    removeindices = [0 EEG.event(first_non_r_peak_idx).latency - EEG.srate];
    removeindices(end+1,:) = [EEG.event(last_non_r_peak_idx).latency + EEG.srate EEG.pnts];
end
